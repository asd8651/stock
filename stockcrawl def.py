        ...
        for o in range(len(otcsid)):
            ...
            if ('data' in allData.keys()):
                ...
                for w in range(len(oday)):
                    insert = (
                    """INSERT  INTO `""" + osid + """` (`Date`, `sid`, `name`, `shareTrades`, `turnover`, `open`, `high`, `low`, `closing`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)""")
                    # ON DUPLICATE KEY UPDATE (`Date`,`sid`,`name`,`shareTrades`,`turnover`,`over`,`high`,`low`,`closing`)VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)有更新，無新增
                    oda = (
                    oday[i][0], osid, oname, oday[i][1], oday[i][2], oday[i][3], oday[i][4], oday[i][5], oday[i][6])
                    cursor.execute(insert, oda)
                    db.commit()
            time.sleep(1)
        db.close()
    def insertONMARKET(self):
        yesterday = datetime.datetime.now().strftime("%Y%m%d")
        odb = pymysql.connect(host='0.tcp.ngrok.io', port=12714, user='root', passwd='ncutim', db='OTC',charset='utf8')
        cursor = db.cursor()
        for l in range(len(newsid)):
            ...
            if ('data' in allData.keys()):
                ...
                for i in range(len(day)):
                    insert = (
                    """INSERT  INTO `""" + sid + """` (`Date`, `sid`, `name`, `shareTrades`, `turnover`, `open`, `high`, `low`, `closing`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)""")
                    # ON DUPLICATE KEY UPDATE (`Date`,`sid`,`name`,`shareTrades`,`turnover`,`over`,`high`,`low`,`closing`)VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)有更新，無新增
